...
q = st.text_input("궁금한 점을 입력하세요 (예: 고혈압이면 어떻게 걸어야 하나요?)")
if st.button("질문 전송") and q.strip():
    matches = find_best_answer(q, KB)
    if matches:
        for m in matches:
            st.markdown(f"**{m}** → {KB[m]}")
    else:
        st.info("관련 정보를 찾지 못했어요. 건강 정보를 입력하고 다시 시도해보세요!")

st.caption("⚠️ 본 챗봇은 교육용 참고자료이며, 개인 건강상태에 따라 전문의 상담이 필요할 수 있습니다.")
