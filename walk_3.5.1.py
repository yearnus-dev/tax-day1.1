...
# Initialize session state storage for plan/progress
...
if st.button("권장 시간 계산 및 4주 루틴 생성"):
    ...
    # Show plan and build a DataFrame for tracking
    rows = []
    for w in plan:
    ...
    # store baseline plan in session_state for later manipulation
    st.session_state['plan_df'] = df
    st.success("4주 루틴이 생성되어 진행관리 섹션에서 확인할 수 있습니다.")

# ------------------ 진행 관리 및 차트 ------------------
st.markdown("---")
st.header("루틴 진행 관리 및 차트")
if st.session_state.get('plan_df') is None:
    st.info("먼저 '권장 시간 계산 및 4주 루틴 생성' 버튼으로 루틴을 생성하세요.")
else:
    plan_df = st.session_state['plan_df']

    # Interactive table: allow user to mark sessions complete and enter actual minutes
    st.subheader("세션별 완료 입력")
    edited = plan_df.copy()
    cols = st.columns([1, 1, 1, 2, 3])
    cols[0].markdown("**주차**")
    cols[1].markdown("**세션**")
    cols[2].markdown("**권장(분)**")
    cols[3].markdown("**실제 완료(분)**")
    cols[4].markdown("**완료 체크**")

    # We'll update session_state in-place by index
    for idx, row in edited.iterrows():
        c1, c2, c3, c4, c5 = st.columns([1, 1, 1, 2, 3])
        c1.write(row['주차'])
        c2.write(int(row['세션번호']))
        c3.write(int(row['권장시간(분)']))
        minutes = c4.number_input(f"actual_minutes_{idx}", min_value=0, max_value=1000, value=int(row['실제완료(분)']), key=f"min_{idx}")
        done = c5.checkbox("완료", value=bool(row['완료여부']), key=f"done_{idx}")
        # apply back
        st.session_state['plan_df'].at[idx, '실제완료(분)'] = int(minutes)
        st.session_state['plan_df'].at[idx, '완료여부'] = bool(done)

    st.markdown("---")
    # Summary statistics and charts
    st.subheader("주간 진행 요약")
    df_now = st.session_state['plan_df'].copy()
    # aggregate per week
    weekly_summary = df_now.groupby('주차').agg(
        권장합계=('권장시간(분)', 'sum'),
        완료합계=('실제완료(분)', 'sum'),
        완료세션수=('완료여부', 'sum'),
        총세션수=('세션번호', 'count')
    ...
    # Progress %
    weekly_summary['진행률(%)'] = (weekly_summary['완료합계'] / weekly_summary['권장합계'] * 100).round(1)
    weekly_summary['진행률(%)'] = weekly_summary['진행률(%)'].fillna(0)

    st.write(weekly_summary)

    # Chart: 막대(권장 vs 완료)
    base = alt.Chart(weekly_summary).transform_fold(
        ['권장합계', '완료합계'], as_=['구분', '분'])
    chart = alt.Chart(weekly_summary).mark_bar().encode(
        x=alt.X('주차:N', title='주차'),
        y=alt.Y('분:Q', title='분'),
        color='구분:N',
        tooltip=['주차', '권장합계', '완료합계', '진행률(%)']
    ).properties(width=700, height=350, title='주간 권장 시간 대비 실제 완료 시간')

    st.altair_chart(chart, use_container_width=True)

    # Line: 누적 진행률
    cumulative = weekly_summary.copy()
    cumulative['누적권장'] = cumulative['권장합계'].cumsum()
    cumulative['누적완료'] = cumulative['완료합계'].cumsum()
    cum_chart = alt.Chart(cumulative).transform_fold(
        ['누적권장', '누적완료'], as_=['구분', '누적분'])
    cum_chart = alt.Chart(cumulative).mark_line(point=True).encode(
        x='주차:N',
        y='누적분:Q',
        color='구분:N',
        tooltip=['주차', '누적권장', '누적완료']
    ).properties(width=700, height=300, title='누적 권장 vs 누적 완료')

    st.altair_chart(cum_chart, use_container_width=True)

    # Allow exporting the updated progress table
    st.markdown("---")
    if st.button("진행상태 CSV로 다운로드"):
        csv = st.session_state['plan_df'].to_csv(index=False).encode('utf-8-sig')
        st.download_button("CSV 다운로드", data=csv, file_name=f"walk_progress_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", mime='text/csv')
